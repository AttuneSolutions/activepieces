...
## Gotchas
- **`.husky/pre-push` hard-blocks any push whose remote ref is `refs/heads/main`** — "Direct pushes to 'main' are not allowed." It fires on forks too, since the hook is committed. Branch + PR is the only path; `SKIP_CHECK=1` is the escape hatch, and it skips the lint/test prompt as well.
- **A non-interactive push silently skips lint and tests.** The same hook only prompts "Run checks before pushing?" when stdout is a TTY; from an IDE, CI, or an agent shell it falls through to `RUN_CHECKS="n"` and prints "Skipping lint and tests." Set `CLAUDE_PUSH=yes` to force the full lint + `test-unit` + `test-api` run on a non-TTY push. So a green local push proves nothing — CI is the real gate.
